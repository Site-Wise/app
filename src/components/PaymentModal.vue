  ...
  >
    ...
    >
      ...
      <form
        ...
        class="flex-1 overflow-y-auto overscroll-contain px-5 sm:px-6 py-5 space-y-4 scroll-smooth-touch"
      >
        ...
        <div class="bg-stone-50 dark:bg-ink-2 rounded-lg p-4 space-y-3">
          ...
          <div v-if="mode !== 'EDIT'">
            <label class="block text-sm font-medium text-stone-700 dark:text-stone-300">{{ t('payments.paymentAccount') }}</label>
          ...
          <div v-else-if="payment?.expand?.account" class="flex justify-between">
            <span class="text-sm font-medium text-stone-700 dark:text-stone-300">{{ t('payments.paymentAccount') }}:</span>
                ...
                <TriStateCheckbox :id="`credit-note-${creditNote.id}`"
                  ...
                  <template #amount-display>
                    <div class="text-right">
                      <span class="text-forest font-medium font-mono tabular-nums">₹{{ creditNote.balance.toFixed(2) }}</span>
          ...
          <div v-if="mode !== 'EDIT'">
            <label class="block text-sm font-medium text-stone-700 dark:text-stone-300">{{ t('payments.paymentDate') }}</label>
          ...
          <div v-if="mode !== 'EDIT'">
            ...
            <div v-if="selectedCreditNoteAmount > 0" class="mt-2 text-sm bg-amber/5 dark:bg-amber/10 p-3 rounded-lg border border-amber/20 dark:border-amber/30">
          ...
          <div v-else>
            ...
            <div class="space-y-2">
              <div class="flex justify-between">
                ...
                <span class="text-sm font-semibold font-mono tabular-nums text-ink dark:text-cream">₹{{ payment?.amount.toFixed(2) }}</span>
              </div>
        ...
        <div v-if="showAllocationProgress"
          ...
          <div class="space-y-2">
            ...
            <div class="flex justify-between text-sm">
              ...
              <span class="text-amber-700 dark:text-amber font-medium font-mono tabular-nums">₹{{ unallocatedAmount.toFixed(2) }}</span>
            </div>

            <!-- Amber progress bar (spec requirement) -->
            <div class="w-full bg-stone-200 dark:bg-ink-4 rounded-full h-2 mt-2 overflow-hidden">
            ...
            <div class="text-xs font-mono tabular-nums text-stone-500 dark:text-stone-400 text-center">
              {{ allocationPercentage }}% {{ t('payments.allocated').toLowerCase() }}
            </div>
          </div>
        </div>
          ...
          <div class="bg-stone-50 dark:bg-ink-2 rounded-lg overflow-hidden">
            <table class="min-w-full divide-y divide-stone-200 dark:divide-ink-4">
              <thead class="bg-stone-100 dark:bg-ink-4">
                <tr>
                  <th class="px-4 py-2 text-left sw-eyebrow text-stone-500 dark:text-stone-400">
                    {{ t('common.type') }}</th>
                  <th class="px-4 py-2 text-left sw-eyebrow text-stone-500 dark:text-stone-400">
                    {{ t('common.date') }}</th>
                  <th class="px-4 py-2 text-right sw-eyebrow text-stone-500 dark:text-stone-400">
                    {{ t('common.amount') }}</th>
                </tr>
              </thead>
        ...
        <div v-if="mode === 'EDIT' && unallocatedAmount <= 0"
          ...
          <div class="flex items-center">
            ...
            <span class="text-sm font-medium text-forest">{{ t('payments.fullyAllocated') }}</span>
          </div>
