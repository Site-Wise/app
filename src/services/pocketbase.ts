...
class AnalyticsSettingService {
  async getAll(): Promise<AnalyticsSetting[]> {
    ...
    return records.map(record => this.mapRecordToAnalyticsSetting(record));
  }
  ...
  async getById(id: string): Promise<AnalyticsSetting | null> {
    ...
    try {
      ...
      return this.mapRecordToAnalyticsSetting(record);
    } catch (error) {
      return null;
    }
  }
  ...
  private mapRecordToAnalyticsSetting(record: any): AnalyticsSetting {
    return {
      id: record.id,
      site: record.site,
      name: record.name,
