 ...
 * Manages service bookings (labor, equipment, professional services)
 */

import { pb } from '../client';
import { getCurrentSiteId, getCurrentUserRole } from '../context';
import { calculatePermissions } from '../types/permissions';
import type { ServiceBooking, PaymentAllocation } from '../types';
import { mapRecordToServiceBooking } from './mappers';

// Forward reference for circular dependency
let paymentAllocationServiceRef: {
  getByServiceBooking: (id: string) => Promise<PaymentAllocation[]>;
} | null = null;

export function setServiceBookingServiceDependencies(deps: {
  paymentAllocationService: { getByServiceBooking: (id: string) => Promise<PaymentAllocation[]> };
}) {
...
export class ServiceBookingService {
  ...
  async delete(id: string): Promise<boolean> {
    ...
    if (!paymentAllocationServiceRef) {
      throw new Error('ServiceBookingService dependencies not initialized');
    }
