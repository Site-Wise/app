/**
 * Common record mappers for PocketBase services
 * These are shared utilities to avoid code duplication across services
 */

import type { RecordModel } from 'pocketbase';
import type {
  User, Site, SiteUser, Account, Item, Vendor, Service, Tag,
  Quotation, ServiceBooking, Delivery, DeliveryItem, Payment,
  PaymentAllocation, AccountTransaction, VendorReturn, VendorReturnItem,
  VendorRefund, VendorCreditNote, CreditNoteUsage, SiteInvitation
} from '../types';
...
export function mapRecordToQuotation(record: RecordModel): Quotation {
  return {
    ...
    quotation_type: record.quotation_type || (record.item ? 'item' : 'service'),
...
export function mapRecordToDelivery(record: RecordModel): Delivery {
  return {
    ...
    notes: record.notes,
    total_amount: record.total_amount,
    rounded_off_with: record.rounded_off_with,
    payment_status: record.payment_status,
    paid_amount: record.paid_amount,
...
// Payment mapper (forward declared to handle circular dependency)
export function mapRecordToPayment(record: RecordModel): Payment {
  return {
    ...
    payment_allocations: record.payment_allocations || [],
    site: record.site,
    created: record.created,
    updated: record.updated,
...
export function mapRecordToVendorReturn(record: RecordModel): VendorReturn {
  return {
    ...
    status: record.status,
    processing_option: record.processing_option,
    notes: record.notes,
    photos: record.photos || [],
...
export function mapRecordToVendorReturnItem(record: RecordModel): VendorReturnItem {
  return {
    ...
    updated: record.updated,
    expand: record.expand ? {
      delivery_item: record.expand.delivery_item ? mapRecordToDeliveryItem(record.expand.delivery_item) : undefined
    } : undefined
  };
}
