 ...
 * Tracks usage of vendor credit notes in payments
 */

import { pb } from '../client';
import { getCurrentSiteId, getCurrentUserRole } from '../context';
import { calculatePermissions } from '../types/permissions';
import type { CreditNoteUsage } from '../types';
import { mapRecordToCreditNoteUsage } from './mappers';
...
let paymentServiceRef: {
  mapRecordToPayment: (record: any) => any;
} | null = null;

export function setCreditNoteUsageServiceDependencies(deps: {
  vendorCreditNoteService: { mapRecordToCreditNote: (record: any) => any };
  paymentService: { mapRecordToPayment: (record: any) => any };
}) {
...
export class CreditNoteUsageService {
  ...
  private mapRecordToUsage(record: any): CreditNoteUsage {
    return {
      id: record.id,
