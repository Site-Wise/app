...
export function setVendorRefundServiceDependencies(deps: {
  ...
  vendorReturnService: { update: (id: string, data: any) => Promise<any> };
}) {
...
export class VendorRefundService {
  ...
  async create(data: Omit<VendorRefund, 'id' | 'site' | 'created' | 'updated'>): Promise<VendorRefund> {
    ...
    if (!accountTransactionServiceRef || !vendorReturnServiceRef) {
      throw new Error('VendorRefundService dependencies not initialized');
    }
